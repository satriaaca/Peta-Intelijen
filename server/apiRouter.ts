...
// 1. DB Health & Stats
apiRouter.get("/db/health", async (_req: Request, res: Response) => {
  try {
    ...
    const annualRes = await pool.query("SELECT COUNT(*) FROM annual_targets");
    res.json({
      status: "ok",
      engine: "Neon PostgreSQL",
      connected: true,
      counts: {
        intelligence: parseInt(intelRes.rows[0]?.count || "0", 10),
        outreach: parseInt(outreachRes.rows[0]?.count || "0", 10),
        caseStats: parseInt(caseStatsRes.rows[0]?.count || "0", 10),
        annualTargets: parseInt(annualRes.rows[0]?.count || "0", 10),
      },
...
apiRouter.get("/intelligence", async (req: Request, res: Response) => {
  try {
...
apiRouter.post("/intelligence", async (req: Request, res: Response) => {
  try {
...
apiRouter.delete("/intelligence/:id", async (req: Request, res: Response) => {
  try {
...
apiRouter.post("/outreach", async (req: Request, res: Response) => {
  try {
    ...
    if (!entry.id || !entry.no || !entry.tema_kegiatan) {
      return res.status(400).json({ error: "Missing required fields (id, no, tema_kegiatan)" });
    }
    ...
    const params = [
      ...
      entry.tema_kegiatan,
      entry.waktu || now.toISOString().split("T")[0],
      entry.tempat || "Tabanan",
      entry.kecamatan || "Tabanan",
...
apiRouter.delete("/outreach/:id", async (req: Request, res: Response) => {
  try {
    const id = req.params.id || req.body?.id || (req.query?.id as string);
    if (!id) {
      return res.status(400).json({ error: "Missing ID" });
...
// Support DELETE /outreach with JSON body or query
apiRouter.delete("/outreach", async (req: Request, res: Response) => {
  try {
...
apiRouter.get("/case-stats", async (req: Request, res: Response) => {
  try {
...
// 9. POST Case Statistics
apiRouter.post("/case-stats", async (req: Request, res: Response) => {
  try {
    const item = req.body;
...
apiRouter.get("/annual-targets", async (req: Request, res: Response) => {
  try {
    const yearFilter = req.query.year ? parseInt(req.query.year as string, 10) : undefined;
    let query = `
      SELECT 
...
// 11. POST Annual Targets
apiRouter.post("/annual-targets", async (req: Request, res: Response) => {
  try {
    const item = req.body;
...
apiRouter.delete("/annual-targets/:id", async (req: Request, res: Response) => {
  try {
    const id = req.params.id || req.body?.id || (req.query?.id as string);
    if (!id) {
      return res.status(400).json({ error: "Missing ID" });
...
// 13. Reset Database
apiRouter.post("/db/reset", async (_req: Request, res: Response) => {
  try {
...
// 14. EXTERNAL JAMPIDUM PERKARA API PROXY
apiRouter.get("/jampidum-perkara", async (req: Request, res: Response) => {
